...
## 2026-08-21 玩家多資源已知限制

- Catalog 已離線覆蓋 17 種資源與 40 組職業／專精拓撲，但這是資料與流程契約，不是全職業全專精的 Retail／PTR 實機簽收。
- 正式服務以 `UnitHasPowerType` 判斷目前客戶端是否提供資源；Catalog 中存在但客戶端回傳不可用的候選會標為 `Unavailable` 並保持隱藏，不會嘗試 raw `UnitPowerMax` 推測。
- 普通可存取值可顯示數值文字；Secret 值只能驅動原生 StatusBar sink，因此文字可能刻意不顯示。這是安全降級，不得以讀回 sink、Tooltip 或估算補值。
- 未採用未經現行公開文件證實的 `StatusBar:SetUnit`、`SetPowerTextFontString`、`SetOnUpdateMode`。若未來 build 新增這些方法，必須重新鎖定 build、生成文件與實機證據後才能評估。
- 專精／變形造成的資源結構重建會在戰鬥中延後；已存在 sink 的百分比更新仍由資源事件驅動。所有職業、專精、變形與戰鬥狀態仍待玩家實機驗證。
