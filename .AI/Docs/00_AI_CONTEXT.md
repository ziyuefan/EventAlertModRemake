...
## 2026-08-21 新根現行基準（優先於歷史段落）

- 專案 Git 根目錄：`D:\Project_EventAlertMod`。
- 可發布插件的唯一來源：`D:\Project_EventAlertMod\EventAlertMod`；插件 ZIP 必須逐檔封裝此資料夾，頂層固定為 `EventAlertMod/`。
- AI 治理資料：`D:\Project_EventAlertMod\.AI`，包含 `Docs/`、`Tools/`、`Data/` 與 `docs_html/`；`Deploy/` 僅保存本機部署／封裝工具；`Dist/` 僅保存本機產物並由 Git ignore。
- 目前 WoW 根目錄：`D:\World of Warcraft`。截至本快照 Retail/PTR 為 `12.1.0.69382`，XPTR 為 `12.0.7.68887`；三個 EventAlertMod 目標目前均為 physical；目前只有 PTR 含 EventAlertMod.toc，Retail/XPTR 尚缺該檔，Test-LocalWoWEnvironment 為 1/3，不能宣稱三通道 ready。Deploy Status/DryRun 三通道通過只代表可安全部署前檢，尚未實際部署。每次部署仍須重新讀取 ProductVersion、Reparse Point 與目標 inventory。
- `Deploy/Deploy-EventAlertMod.ps1` 的現行治理為 Registry 優先、`Deploy/DeploymentTargets.json` fallback、可由使用者確認或 `-WowRoot` 改用其他 WoW 根目錄；Status 必須列出 Retail/PTR/XPTR 三通道完整 ProductVersion。互動選 PTR/XPTR 時才詢問是否納入 Retail；noninteractive 不得暗中加入 Retail；任何 Reparse Point 都 fail-closed。
- `D:\EventAlertMod` 僅是歷史路徑名稱，不是本新結構的存取目標；本節之後的歷史段落若仍出現舊路徑，只作過往證據保存，不得據此部署或修改。
