...
- 擁有查找緩存
- 必須避免激烈的戰鬥查詢循環
## Data/PlayerResourceCatalog + Services/PlayerResourceCapability + Services/PlayerResourceService

輸入：

- `PlayerResourceCatalog` 的 17 種資源定義與 13 職業／40 組職業專精拓撲
- `UnitHasPowerType("player", powerType)` 的安全布林能力
- 玩家資源、專精、天賦、變形、進出世界與模組設定事件
- `EAM_DB.profiles.classes[classToken].resources` 的 class/spec 有效設定

輸出：

- 每個目前可用且啟用的資源各自一個穩定 runtime descriptor；同一專精可同時追蹤多種資源
- `Numeric`、`Secret`、`Unavailable` capability 分類；分類只決定渲染通道，不輸出 raw current／max／percent
- O(1) power-token dispatcher、符文專用事件與變形／專精重建要求；禁止 per-resource `OnUpdate`
- 透過 `UI/PowerRenderer` 預建的專用 StatusBar sink 顯示百分比；`Services/ClassPowerService` 只保留相容 facade，不再擁有單一資源選擇邏輯

狀態與 Secret 邊界：

- `UnitPowerPercent(..., CurveConstants.ZeroToOne)` 回值只可在同一呼叫鏈單向送入 `StatusBar:SetValue`，隨即清除 local；禁止保存、比較、字串化、序列化、作 table key 或以 getter 讀回。
- 普通可存取數值才可顯示文字；Secret 資源只顯示 C-side sink。不得使用未經官方文件證實的 `StatusBar:SetUnit`、`SetPowerTextFontString` 或 `SetOnUpdateMode`。
- 結構建立與重排只在非戰鬥進行；既有預建 sink 的數值更新可由資源事件驅動。模組停用時先 gate API，再隱藏並清除 runtime descriptor。
- `Debug/PromptExport` 只輸出 key、powerType、token、capability、前景設定與計數，不輸出 current／max／percent。
