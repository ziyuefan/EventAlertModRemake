...
| `.AI/Tools/Run-FlowValidation.ps1` | 開發端 | 執行 suite，輸出 JSON／Markdown | 否 |
| `.AI/Tools/Import-EAMFlowReport.ps1` | 開發端 | 匯入 WTF 或 JSON，驗證 Flow／Live schema、重算 summary 與 raw build flags、核對 client 身分並拒絕隱私值 | 否 |
| `.AI/Tools/Test-ValidationContracts.ps1` | 開發端 | 驗證 JSON Schema、21 點／37 案 Lua 同步、continuity drift、PowerShell AST 與匯入器正反例 | 否 |
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\.AI\Tools\CheckLuaSyntax.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File .\.AI\Tools\Run-FlowValidation.ps1 -Suite all
...
```powershell
.\.AI\Tools\Run-FlowValidation.ps1 -Suite quick
.\.AI\Tools\Run-FlowValidation.ps1 -Suite core
.\.AI\Tools\Run-FlowValidation.ps1 -Suite boundary
.\.AI\Tools\Run-FlowValidation.ps1 -Suite all
```

輸出位於 `.AI/TestResults/`，預設不納入 Git。若任何案例為 `failed` 或 `pending`，工具回傳 exit code 1。
...
工具以 WoW 主程式 `ProductVersion` 比對預期 patch train，並檢查所測通道的 `Interface\AddOns\EventAlertMod` 是無巢狀 Reparse Point且含 TOC 的完整實體目錄。失敗時回傳 exit code 1，禁止繼續實機簽收；完整 build 與目標證據輸出至 `.AI/TestResults/`。
...
匯入前必須先確認測試使用的客戶端版本，不得從 Classic 版本樹取證，也不得將帳號或角色名稱寫入文件與報告。2026-08-21 三個 AddOn 目標均為實體目錄，但只有 PTR 含 TOC；Retail／XPTR 完成部署並通過環境斷言前不得簽收。回灌流程只讀 WTF，不修改 AddOn 目標；完整規則見 `.AI/Docs/27_LOCAL_WOW_ENVIRONMENT.md`。

匯入命令：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass `
    -File .\.AI\Tools\Import-EAMFlowReport.ps1 `
...
- `.AI/Tools/Test-LocalWoWEnvironment.ps1` 同日通過 3/3：`_retail_` 12.0.7、`_ptr_` 12.1.0、`_xptr_` 12.0.7，三個 EAM Reparse Point 均指向實體專案。
...
- 新增 `.AI/Docs/28_PROJECT_CONTINUITY.md`、`.AI/Data/ProjectContinuity.json` 與嚴格 Schema；`.AI/Tools/Test-ValidationContracts.ps1` 負責 snapshot、ID、issue、矩陣與隱私漂移檢查。
...
- 開發端只讀匯入統一使用 `.AI/Tools/Import-EAMFlowReport.ps1 -Path '<使用者明確提供的檔案>' -ReportType Flow|Live|UnitPower`；不得列舉 Account／角色目錄，也不得把本機絕對路徑寫入專案文件或報告。
...
- `EAM_SVG_CAPABILITY_REPORT_JSON` 列入 SavedVariables；`.AI/Tools/Import-EAMFlowReport.ps1` 的 `ReportType SVG` 依 kind 重算 capability、case、status，且 mutation 測試拒絕假 Vector HasSVG、缺 Texture lifecycle 與非法 Texture introspection。
...
## 2026-08-21 玩家多資源與封裝前檢

本輪新增／更新的離線案例：

- `unitpower.combat_native_sink_no_raw_reads`
- `unitpower.secondary_and_secret_sink`
- `unitpower.catalog_and_class_matrix`
- `unitpower.saved_variables_scope_roundtrip`
- `aura121.saved_variables.migration_v1_v6`

完整 `all` 為 70/70，artifact 為 `.AI/TestResults/EAM_FlowValidation_all_20260821_043946.json`。`Deploy/Build-Package.ps1` 會先跑 AddOn Lua、Flow all、Validation Contracts，任一非零立即停止，不建立 ZIP。

Flow 會更新「最新報告選擇」所使用的 TestResults artifact；因此開發端不得把 Flow 與 Contracts 並行執行。正確順序是 Flow 完成並關閉檔案後再執行 Contracts。此規則是離線測試編排要求，不代表玩家資源已在 Retail／PTR 視覺通過。
