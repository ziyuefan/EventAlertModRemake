...
- aura121.saved_variables.migration_v1_v6：v1 至 v6 遷移、v4 全域 backup、active class／隔離 profile 與 v5 玩家資源旗標 backup。
...
## 2026-08-14 Alpha 5：Profile、字型與語系回歸

離線新增／維持下列 gate：

- profile.codec.roundtrip、profile.codec.reject、profile.codec.preview_no_side_effect、profile.codec.merge_replace：驗證 EAMAP1 canonical JSON／Base64、checksum、拒絕 malformed／future schema／duplicate／過大輸入、preview 零副作用與戰鬥 guard。
- ui.text_layout.font_family：驗證四種字型白名單、路徑映射、no-op revision 與非法值回退。
- locale.dynamic.buttons 與 spec menu refresh：驗證切換語系後 EAM 自有按鈕、下拉、條件與專精選單重新取 EAM.L，且 Auto Detect 固定英文。
...
## 2026-08-21：玩家多資源正式路徑

離線驗證必須覆蓋：

- `unitpower.catalog_and_class_matrix`：17 種資源與 13 職業／40 組專精拓撲，德魯伊同時包含 Mana、Rage、Energy、ComboPoints、LunarPower。
- `unitpower.combat_native_sink_no_raw_reads`：戰鬥內資源事件可直接更新預建 sink，`UnitPower`／`UnitPowerMax` raw read、getter 與 Secret scalar operation 均為 0。
- `unitpower.secondary_and_secret_sink`：普通與 Secret 百分比都走允許的 C-side sink；只有普通安全數字可顯示文字。
- `unitpower.saved_variables_scope_roundtrip`：class/spec 繼承、覆寫、reset、no-op revision 與事件恰一次。
- `aura121.saved_variables.migration_v1_v6`：完整遷移到 schema v6，並保留 v5 玩家資源旗標備份。

Retail／PTR 玩家實測至少包含法師法力、戰士怒氣、野性德魯伊能量＋連擊點、暗影牧師瘋狂值、死亡騎士符文＋符能，並逐職業／專精檢查所有 `UnitHasPowerType` 可用資源。每項都要驗證脫戰與戰鬥中的產生、消耗、歸零，前景／背景／數值文字、個別位置與透明度、專精覆寫／重設、變形／專精切換、模組停用／重啟，以及 Lua error、taint、blocked action。Capability probe 的 `accepted` 只證明 setter 未拒絕，不能替代正式面板的目視 pass。

本輪最新離線結果為 Lua syntax `64/64`（62 AddOn + 2 offline tests）、Flow all `75/75`、Flow boundary `54/54`、Validation Contracts `436/436`；仍不得標示任何客戶端的玩家資源實機通過。

## 2026-08-21：玩家職業資源設定即時生命週期

### 離線契約

- unitpower.config_immediate_lifecycle：非戰鬥修改有效設定後立即重建；戰鬥修改只保存並標記 combatRebuildDeferred；PLAYER_REGEN_ENABLED 後只重建一次，停用後舊資源 frame 不復活。
- unitpower.catalog_and_class_matrix：17 種資源、13 職業與 40 組職業／專精拓撲；同一專精可同時追蹤多資源。
- unitpower.combat_native_sink_no_raw_reads：UnitPower／UnitPowerMax raw read、getter、Secret scalar operation 均為 0，Secret 只直接送入 write-only sink。
- migration fixture 必須驗證 class/spec override 的有效合併，不要求未覆寫的 classDefaults.settings 預先產生每項資源冗餘表。

### 玩家實機（REQUIRES_WOW_12_1_RUNTIME）

1. 脫戰開啟玩家資源設定，改變一項外觀或啟用狀態；不要 /reload，確認畫面立即更新。
2. 進入戰鬥後改同一設定，確認設定先保存但結構性變更不在戰鬥中重建；離開戰鬥後應一次套用。
3. 依序測法師 Mana、戰士 Rage、野性德魯伊 Energy／ComboPoints、暗影牧師 Insanity、死亡騎士 Runes／RunicPower，再逐一檢查其他職業／專精可用資源。
4. 觀察 NUMERIC 的文字、percent、threshold、fullGlow；SECRET_DISPLAY 只驗狀態列視覺，不要求 raw 數字。
5. 切專精、德魯伊變形、UNIT_MAXPOWER 或符文變化後，確認只更新受影響資源，不拆除其他候選。
6. 回報 clientChannel、patch、build、interface、戰鬥狀態、是否手動 /reload、visualObservation、Lua error／taint／blocked action／Forbidden access。

## 2026-08-21：玩家資源 completion audit 與 sampler 邊界

- 最新離線 artifact：Lua `64/64`、Flow all `75/75`、Flow boundary `54/54`、Validation Contracts `436/436`；來源分別為 `.AI/TestResults/EAM_FlowValidation_all_20260821_094159.json` 與 `.AI/TestResults/EAM_FlowValidation_boundary_20260821_094201.json`。
- 已驗證契約包含 `PlayerResourceProbe` 逐資源 metadata、`unitpower.legacy_120007_adapter`、Druid form／多資源 topology、anchor／position、module event unregister、Secret source-to-sink 與設定即時／戰鬥延後生命週期。
- `unitpower.background_sampler_gate`：只有 probe 明確標記事件缺失才啟用單一 demand-driven sampler；驗證同 generation 只排一個 task、事件恢復後停止、模組停用清理、戰鬥延後與 stale generation 不得執行。專項 Flow／Contract 已離線通過，但不代表 runtime 事件或視覺通過。
- 真人補測：Retail／PTR 12.1 與 XPTR 12.0.7 分別手動啟動 ResourceProbe；依序測 Mana、Rage、Druid Caster／Cat／Bear／Moonkin、Insanity、Runes／RunicPower，再逐職業／專精檢查可用資源，回報 eventObserved、visualObservation、build、Interface、戰鬥狀態與 `/reload`。
- 實機必確認事件正常時不啟用 sampler；只有玩家明確標記事件缺失才可觀察 sampler，並記錄其啟停、戰鬥延後與停止時的事件解除註冊。離線通過不得升格為 PTR／XPTR／Retail pass。