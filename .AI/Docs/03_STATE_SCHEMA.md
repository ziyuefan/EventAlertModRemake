...
## 2026-08-21 現行 schema v6：玩家多資源設定

- `EAM_DB.schemaVersion=6`。v5 升級時先把舊 `config.power*` 旗標保存到 `migrationBackups.playerResourceFlagsV5`，再建立資源設定；不得丟棄舊值或把 runtime sink 寫入 SavedVariables。
- 每個職業 profile 新增 `resources = { schema = 3, globalDefaults = {}, classDefaults = {}, specs = {} }`。`globalDefaults` 提供全域資源外觀基線；`classDefaults[resourceKey]` 保存職業預設；`specs[specID][resourceKey]` 只保存該專精覆寫，未覆寫欄位繼承 classDefaults。
- 每項資源可保存 `enabled`、`displayMode`、`showForeground`、`showBackground`、`showValue`、`showPercent`、`fullGlow`、`fontFamily`、`fontSize`、`orientation`、`anchor`、`position`、`threshold`、`offsetX`、`offsetY`、`scale`、`alpha`、`foregroundAlpha`、`backgroundAlpha`、`order`、`barWidth`、`barHeight`、`iconSize`、`spacing`。所有欄位都經白名單、型別與範圍正規化。
- `SavedVariables.updatePlayerResourceConfig()` 是唯一正式寫入口：無變更不增加 revision；真變更只增加一次 revision，並發出 `EAM_PLAYER_RESOURCE_CONFIG_CHANGED`。非戰鬥中由 PlayerResourceService 立即重建並回報 `configApplied`；戰鬥中只保存設定並標記 `combatRebuildDeferred`，於 `PLAYER_REGEN_ENABLED` 合併重建一次。`resetToClass` 只移除指定專精的覆寫，不改其他職業或專精。
- Catalog、capability、active sink、目前百分比、current／max 與事件快取都是 session/runtime 狀態，絕不可寫入 SavedVariables、報告或 profile codec。

### 2026-08-21 玩家資源設定生命週期補充

- 非戰鬥中，SavedVariables.updatePlayerResourceConfig 的真變更會送出 EAM_PLAYER_RESOURCE_CONFIG_CHANGED，PlayerResourceService 立即重建 topology 並再次套用有效設定；/reload 只用於載入新程式或驗證保存。
- 戰鬥中，desired config 先保存，service 回報 combatRebuildDeferred；PLAYER_REGEN_ENABLED 合併多次變更後只重建一次。
- runtime status 的 foregroundPowerType／foregroundToken 只描述目前前景 metadata；activePowerType／activePowerToken 保留為相容欄位，不代表只追蹤一項資源。
- threshold、showPercent、fullGlow、fontFamily、fontSize、orientation 會經有效設定傳到 PowerRenderer；threshold 與 fullGlow 只對 NUMERIC 顯示能力產生 Lua 視覺效果。
- 最新離線證據：Lua syntax 64/64（62 AddOn + 2 offline tests）、Flow all 75/75、Flow boundary 54/54、Validation Contracts 436/436；Retail／PTR／XPTR 真人簽收仍需玩家操作。

## 2026-08-21 玩家資源 v6 完成審計補充

- 玩家資源設定 schema 3 的每項資料包含 `anchor` 與 `position`；前者為父框架錨點，後者為資源框架定位點，兩者均受白名單驗證並搭配 offset／scale。
- `PlayerResourceProbe` 只保存 transient 的 tracked／available／foreground／background／capability／sinkAvailable／eventObserved metadata，不保存 UnitPower、UnitPowerMax、percent 或其他 Secret 原值。
- XPTR 12.0.7 由 `PlayerResourceCapability.getClientAdapter()` 進入 numeric legacy adapter；不得呼叫 12.1 Secret sink。這是相容契約，不代表實機通過。
- 只有 probe 確認事件缺失時才允許單一背景 sampler；`unitpower.background_sampler_gate` 與靜態 contract 已通過離線驗證，仍待實機簽收，不能將其執行結果寫成實機證據。
- sampler 的 generation、事件缺失旗標、排程 handle、capability 綁定策略與 `eventObserved` 都是 session/runtime 狀態，不得寫入 SavedVariables、Profile 或可複製報告；報告僅保留安全布林與 metadata。