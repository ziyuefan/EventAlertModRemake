...
檔案: .AI\Tools\Run-FlowValidation.ps1

理念:
- 執行 .AI/Tests/FlowValidationHarness.lua，產生可回灌的 JSON 與 Markdown 離線證據。
- Harness 從 EventAlertMod/ 載入正式插件模組。

邊界:
- Mock 通過不代表 WoW Retail／PTR／XPTR 實機通過。
- 不連網、不讀 WTF、不修改 AddOn 原始碼。
#>
[CmdletBinding()]
param(
...
$harness = Join-Path $paths.GovernanceRoot "Tests\FlowValidationHarness.lua"
...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $reportDirectory = $paths.TestResultsRoot
} elseif ([System.IO.Path]::IsPathRooted($OutputDirectory)) {
    $reportDirectory = $OutputDirectory
} else {
    $reportDirectory = Join-Path $paths.ProjectRoot $OutputDirectory
}
...
foreach ($case in $report.cases) {
    $lines.Add("| $(Escape-MarkdownCell $case.id) | $(Escape-MarkdownCell $case.suite) | $(Escape-MarkdownCell $case.status) | $($case.durationMs) | $(Escape-MarkdownCell $case.message) |")
}
$lines.Add("")
$lines.Add("## 開發回灌")
$lines.Add("")
