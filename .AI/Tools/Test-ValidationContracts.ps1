...
$projectRoot = $eamPaths.ProjectRoot
$governanceRoot = $eamPaths.GovernanceRoot
$root = $eamPaths.AddonRoot
$testResultsRoot = $eamPaths.TestResultsRoot
...
$paths = @{
    PlacementData = Join-Path $governanceRoot "Data\TextPlacementContract.json"
    PlacementSchema = Join-Path $governanceRoot "Schemas\EAM_TextPlacementContract.schema.json"
    MatrixData = Join-Path $governanceRoot "Data\LiveValidationMatrix.json"
    MatrixSchema = Join-Path $governanceRoot "Schemas\EAM_LiveValidationMatrix.schema.json"
    ContinuityData = Join-Path $governanceRoot "Data\ProjectContinuity.json"
    ContinuitySchema = Join-Path $governanceRoot "Schemas\EAM_ProjectContinuity.schema.json"
    ContinuityDoc = Join-Path $governanceRoot "Docs\28_PROJECT_CONTINUITY.md"
    IssueLog = Join-Path $governanceRoot "Docs\15_DEVELOPMENT_ISSUE_LOG.md"
    Agents = Join-Path $governanceRoot "AGENTS.md"
    Context = Join-Path $governanceRoot "Docs\00_AI_CONTEXT.md"
    LiveFixture = Join-Path $governanceRoot "Tests\Fixtures\EAM_LiveValidationReport.incomplete.json"
    LiveSchema = Join-Path $governanceRoot "Schemas\EAM_LiveValidationReport.schema.json"
    FlowSchema = Join-Path $governanceRoot "Schemas\EAM_FlowValidationReport.schema.json"
    ...
    HarnessLua = Join-Path $governanceRoot "Tests\FlowValidationHarness.lua"
    ...
    SVGSchema = Join-Path $governanceRoot "Schemas\EAM_SVGCapabilityReport.schema.json"
    SVGFixture = Join-Path $governanceRoot "Tests\Fixtures\EAM_SVGCapabilityReport.incomplete.json"
    SVGObservedFixture = Join-Path $governanceRoot "Tests\Fixtures\EAM_SVGCapabilityReport.ptr69189-observed.json"
...
if (-not $FlowReport) {
    $latestFlow = Get-ChildItem -LiteralPath (Join-Path $testResultsRoot ".") -Filter "EAM_FlowValidation_all_*.json" |
...
$missingResourceLocaleKeys = @()
foreach ($localeName in $localeNames) {
    $localePath = Join-Path $root ("Locale\" + $localeName + ".lua")
    $localeContent = [System.IO.File]::ReadAllText($localePath)
    foreach ($resourceLocaleKey in $resourceLocaleKeys) {
        $assignmentCount = [regex]::Matches(
            $localeContent,
            '(?m)^L\.' + [regex]::Escape($resourceLocaleKey) + '\s*='
        ).Count
        if ($assignmentCount -ne 1) {
            $missingResourceLocaleKeys += ($localeName + ":" + $resourceLocaleKey + ":" + $assignmentCount)
        }
    }
}
Assert-Contract (
    $resourceLocaleKeys.Count -ge 50 -and
    $missingResourceLocaleKeys.Count -eq 0
) "Five locales cover every player resource panel and resource-name key exactly once" ($missingResourceLocaleKeys -join ", ")
Assert-Contract (
...
Assert-Contract (
    ...
    $cooldownServiceSource.Contains('function CooldownService.onVisualTimerExpired') -and
    $cooldownServiceSource.Contains('function CooldownService.onCombatEvent')
) "Cooldown activation is gated by exact player cast and service-owned timer refresh"
Assert-Contract (
    $cooldownServiceSource.Contains('local behaviorRemove = resolveBehavior(alert, "cooldownRemoveAura")') -and
    $cooldownServiceSource.Contains('local behaviorOutside = resolveBehavior(alert, "showSCDOutsideCombat")') -and
    $cooldownServiceSource.Contains('local behaviorGlow = resolveBehavior(alert, "glowSCDWhenUsable")') -and
    $cooldownServiceSource.Contains('state.usableGlow = state.completed and behaviorGlow or false') -and
...
Assert-Contract (
    ...
    $savedVariablesSource.Contains('EAM_COOLDOWN_CONFIG_CHANGED') -and
    $savedVariablesSource.Contains('if type(options[field]) == "boolean"') -and
    $savedVariablesSource.Contains('if record[field] ~= nil then')
) "Cooldown behavior mutation and profile records preserve explicit false"
Assert-Contract (
    $profileCodecSource.Contains('allowed[COOLDOWN_BEHAVIOR_FIELDS[index]] = true') -and
    $profileCodecSource.Contains('if type(record[field]) ~= "boolean"') -and
    $profileCodecSource.Contains('if type(alert[field]) == "boolean"')
) "Profile codec uses an explicit cooldown behavior whitelist"
Assert-Contract (
    $optionsSource.Contains('function Options.refreshCooldownBehaviorControls') -and
    $optionsSource.Contains('savedVariables.updateCooldownBehavior') -and
    $optionsSource.Contains('self.eamValue = nil') -and
    $optionsSource.Contains('EAM_OPT_COND_CD_OVERRIDE_OFF')
) "Options exposes tri-state cooldown behavior controls"
Assert-Contract (
...
$playerResourcePanelSource = [System.IO.File]::ReadAllText((Join-Path $root "UI\PlayerResourcePanel.lua"))
$playerResourceProbeSource = [System.IO.File]::ReadAllText((Join-Path $root "Debug\PlayerResourceProbe.lua"))
$tooltipMonitorSource = [System.IO.File]::ReadAllText((Join-Path $root "Services\TooltipMonitorService.lua"))
$slashSource = [System.IO.File]::ReadAllText((Join-Path $root "UI\Slash.lua"))
$tooltipMenuSource = [System.IO.File]::ReadAllText((Join-Path $root "UI\TooltipMonitorMenu.lua"))
...
Assert-Contract (
    ...
    $tooltipMonitorSource.Contains("lastTryOpenReason") -and
    $tooltipMonitorSource.Contains("lastModifierKey") -and
    $tooltipMonitorSource.Contains("auraCVarReadResult") -and
    $tooltipMonitorSource.Contains("auraCVarSetResult") -and
    $tooltipMonitorSource.Contains("openManualTargetAuraMenu") -and
    -not $tooltipMonitorSource.Contains("GetMouseFocus") -and
    -not $tooltipMonitorSource.Contains("GetChildren")
) "Target Aura diagnostics are anonymous and avoid frame traversal"

Assert-Contract (
    $slashSource.Contains('string.match(input, "^%s*add%s+target%s*$")') -and
    $slashSource.Contains("openManualTargetAuraMenu") -and
    $tooltipMenuSource.Contains('source.kind == "aura"') -and
    $tooltipMenuSource.Contains("ACTION_AURA_TARGET")
) "Explicit manual target aura popup route is reachable"

Assert-Contract (
    $flowRunnerSource.Contains("auraDiagnosticsValid") -and
    $flowRunnerSource.Contains("manualTargetRouteValid") -and
...
Assert-Contract (
    ...
    $resourceCatalogSource.Contains('Catalog.ResourceCount = #definitions') -and
    $resourceCatalogSource.Contains('DRUID = keys(MANA, RAGE, ENERGY, COMBO, LUNAR)') -and
    $resourceCatalogSource.Contains('[102] = keys(MANA, RAGE, ENERGY, COMBO, LUNAR)') -and
    $resourceCatalogSource.Contains('[103] = keys(MANA, RAGE, ENERGY, COMBO, LUNAR)') -and
    $resourceCatalogSource.Contains('[104] = keys(MANA, RAGE, ENERGY, COMBO, LUNAR)') -and
    $resourceCatalogSource.Contains('[105] = keys(MANA, RAGE, ENERGY, COMBO, LUNAR)') -and
    $resourceSpecScenarioCount -eq 40
...
Assert-Contract (
    ...
    $playerResourceServiceSource.Contains("draw.applySecretPercent") -and
    $playerResourceServiceSource.Contains("percent = nil") -and
    $powerRendererSource.Contains("pcall(frame.statusBar.SetValue, frame.statusBar, percent)") -and
    $powerRendererSource.Contains("pcall(frame.statusBar.SetValue, frame.statusBar, 0)") -and
    -not $powerRendererSource.Contains("GetValue(") -and
    -not $powerRendererSource.Contains("GetMinMaxValues(") -and
    -not $powerRendererSource.Contains('SetScript("OnUpdate"')
) "Secret player resources use dedicated write-only native sinks without readback or polling"
Assert-Contract (
    $promptExportLua.Contains('EAM.Services.PlayerResourceService') -and
    $promptExportLua.Contains('powerStatus.resources') -and
    $promptExportLua.Contains('"trackedResourceCount"') -and
    $promptExportLua.Contains('"resources"') -and
    $promptExportLua.Contains('"rawValuesExposed": false') -and
    -not $promptExportLua.Contains('powerStatus.selectedFrom') -and
    -not $promptExportLua.Contains('powerStatus.predicateAvailable') -and
    -not $promptExportLua.Contains('powerStatus.safeUpdateCount') -and
    -not $promptExportLua.Contains('powerStatus.secretUpdateCount')
) "Prompt export reports multi-resource safe metadata without raw power values"

Assert-Contract (
    $playerResourceProbeSource.Contains('type = "EAM_PLAYER_RESOURCE_PROBE_REPORT"') -and
    $playerResourceProbeSource.Contains('rawValuesCollected = false') -and
    $playerResourceProbeSource.Contains('key = source.key') -and
    $playerResourceProbeSource.Contains('powerType = source.powerType') -and
    $playerResourceProbeSource.Contains('tracked = source.tracked == true') -and
    $playerResourceProbeSource.Contains('available = source.available == true') -and
    $playerResourceProbeSource.Contains('foreground = source.foreground == true') -and
    $playerResourceProbeSource.Contains('background = source.background == true') -and
    $playerResourceProbeSource.Contains('capability = source.capability') -and
    $playerResourceProbeSource.Contains('sinkAvailable = sinkAvailable(source.capability)') -and
    $playerResourceProbeSource.Contains('eventObserved = eventObservationCount > 0') -and
    $playerResourceProbeSource.Contains('eventObservationCount = eventObservationCount') -and
    ...
    -not $playerResourceProbeSource.Contains('SetScript("OnUpdate"')
) "Player resource probe exports safe per-resource metadata without raw Secret values or polling"
Assert-Contract (
    $playerResourceProbeSource.Contains('router.register("UNIT_POWER_UPDATE"') -and
    $playerResourceProbeSource.Contains('router.unregister("UNIT_POWER_UPDATE"') -and
    $playerResourceProbeSource.Contains('router.register("UPDATE_SHAPESHIFT_FORM"') -and
    $playerResourceProbeSource.Contains('router.unregister("UPDATE_SHAPESHIFT_FORM"') -and
    $unitPowerProbeSource.Contains('resourceProbe.start()') -and
    $unitPowerProbeSource.Contains('resourceProbe.stop()') -and
    $playerResourceTocSource.Contains('Debug\PlayerResourceProbe.lua') -and
    $flowHarnessSource.Contains('loadModule("EventAlertMod/Debug/PlayerResourceProbe.lua")')
) "Player resource probe follows player-operated start-stop event lifecycle and load order"
Assert-Contract (
    $flowRunnerSource.Contains('id = "unitpower.resource_probe_safe_metadata"') -and
    $flowRunnerSource.Contains('report.schema == 2') -and
    $flowRunnerSource.Contains('report.rawValuesCollected == false') -and
    $flowRunnerSource.Contains('rage.eventObservationCount == 1') -and
    ...
    [regex]::Matches($playerResourceServiceSource, 'Scheduler\.after\(').Count -eq 1 -and
    -not $playerResourceServiceSource.Contains('SetScript("OnUpdate"') -and
    -not $playerResourceServiceSource.Contains('pcall(api.UnitPower,') -and
    -not $playerResourceServiceSource.Contains('pcall(api.UnitPowerMax,') -and
    $powerRendererSource.Contains('numericTextSink = setNumericTextFormatted') -and
    $powerRendererSource.Contains('frame.valueText:SetFormattedText') -and
...
) "Probe-gated background sampler has one shared Scheduler path, combat and stale-generation Flow, and zero raw Secret reads"
Assert-Contract (
    $constantsLua.Contains("PLAYER_RESOURCE_SCHEMA_VERSION = 4") -and
    $savedVariablesSource.Contains("normalizePlayerResourceSetting") -and
    $savedVariablesSource.Contains("showPercent = true") -and
    $savedVariablesSource.Contains("fullGlow = true") -and
    $savedVariablesSource.Contains("function SavedVariables.updatePlayerResourceConfig") -and
    $savedVariablesSource.Contains("valueFontSize = 14") -and
    $savedVariablesSource.Contains("valueOffsetX = 0") -and
    $savedVariablesSource.Contains("valueOffsetY = 0") -and
    $savedVariablesSource.Contains('field == "resetToClass"') -and
    $savedVariablesSource.Contains('"EAM_PLAYER_RESOURCE_CONFIG_CHANGED"') -and
    $playerResourceServiceSource.Contains('eventName == "EAM_PLAYER_RESOURCE_CONFIG_CHANGED"') -and
    $playerResourceServiceSource.Contains('"configChanged"') -and
    $playerResourceServiceSource.Contains('"combatRebuildDeferred"') -and
    $playerResourceServiceSource.Contains('規格例外') -and
...
Assert-Contract (
    ...
    $playerResourcePanelSource.Contains('"showBackground"') -and
    $playerResourcePanelSource.Contains('"showValue"') -and
    $playerResourcePanelSource.Contains('"showPercent"') -and
    $playerResourcePanelSource.Contains('"fullGlow"') -and
    $playerResourcePanelSource.Contains('"valueFontSize"') -and
    $playerResourcePanelSource.Contains('"valueOffsetX"') -and
    $playerResourcePanelSource.Contains('"valueOffsetY"') -and
    $playerResourcePanelSource.Contains('EAM_RESOURCE_VALUE_FONT_SIZE') -and
    $playerResourcePanelSource.Contains("Capability.classify(definition)") -and
    $optionsSource.Contains('localized("EAM_RESOURCE_OPEN"') -and
    $playerResourceTocSource.Contains("UI\PlayerResourcePanel.lua")
) "Options exposes capability-aware independent player resource settings"
Assert-Contract (
    $playerResourceServiceSource.Contains('showPercent = saved and saved.showPercent == true or false') -and
    $playerResourceServiceSource.Contains('valueFontSize = boundedNumber') -and
    $playerResourceServiceSource.Contains('valueOffsetX = boundedNumber') -and
    $playerResourceServiceSource.Contains('valueOffsetY = boundedNumber') -and
    $playerResourceServiceSource.Contains('node.config.fullGlow') -and
    $powerRendererSource.Contains('local TextPlacement = EAM.UI.TextPlacement') -and
    $powerRendererSource.Contains('function PowerRenderer.applyNumeric(resourceKey, powerType, currentValue, maximumValue, percent, showValue, showPercent, fullGlow, threshold)') -and
    $powerRendererSource.Contains('frame.glow:Hide()') -and
    $powerRendererSource.Contains('valueFontSize') -and
    $powerRendererSource.Contains('valueOffsetX') -and
    $powerRendererSource.Contains('valueOffsetY') -and
    $powerRendererSource.Contains('frame.valueText:SetPoint("CENTER", frame.icon')
) "Player resource config fields reach the renderer without reload"

Assert-Contract (
    $savedVariablesSource.Contains('local PLAYER_RESOURCE_ANCHOR_POINTS = freeze({') -and
    $savedVariablesSource.Contains('"anchor",') -and
    $savedVariablesSource.Contains('"position",') -and
    $savedVariablesSource.Contains('anchor = "TOPLEFT"') -and
    $savedVariablesSource.Contains('position = "TOPLEFT"') -and
    $savedVariablesSource.Contains('field == "anchor" or field == "position"') -and
    $playerResourcePanelSource.Contains('local POINT_OPTIONS = freeze({') -and
    $playerResourcePanelSource.Contains('cyclePoint("anchor")') -and
    $playerResourcePanelSource.Contains('cyclePoint("position")') -and
    $playerResourceServiceSource.Contains('anchor = saved and saved.anchor or "TOPLEFT"') -and
    $playerResourceServiceSource.Contains('position = saved and saved.position or "TOPLEFT"') -and
    $powerRendererSource.Contains('local position = config and VALID_ANCHOR_POINTS[config.position]') -and
    $powerRendererSource.Contains('local anchorPoint = config and VALID_ANCHOR_POINTS[config.anchor]') -and
    $flowRunnerSource.Contains('energyClass.anchor == "TOPRIGHT"') -and
...
Assert-Contract (
    ...
    $flowRunnerSource.Contains('id = "unitpower.catalog_and_class_matrix"') -and
    $flowRunnerSource.Contains('id = "unitpower.saved_variables_scope_roundtrip"')
) "Flow covers combat native sinks, independent resources, and the class/spec catalog"

Assert-Contract (
    $playerResourcePanelSource.Contains('"threshold"') -and
    $playerResourcePanelSource.Contains('EAM_RESOURCE_THRESHOLD') -and
    $playerResourcePanelSource.Contains('Panel.controls.threshold:SetEnabled(numericCapability)')
) "Player resource panel exposes a numeric-only threshold control"

Assert-Contract (
    $playerResourceServiceSource.Contains('function PlayerResourceService.getForegroundPowerType()') -and
    $playerResourceServiceSource.Contains('function PlayerResourceService.getForegroundToken()') -and
    $playerResourceServiceSource.Contains('foregroundPowerType = PlayerResourceService.foregroundPowerType') -and
    $playerResourceServiceSource.Contains('tracked = true') -and
    $resourceCatalogSource.Contains('Catalog.ClassResourceKeys')
) "Player resource topology keeps tracked candidates independent from foreground metadata"

Assert-Contract (
    $promptExportLua.Contains('"foregroundPowerType"') -and
    $promptExportLua.Contains('"foregroundToken"')
) "Player resource reports expose foreground metadata without changing raw-value boundary"

Assert-Contract (
    $flowRunnerSource.Contains('id = "unitpower.config_immediate_lifecycle"') -and
    $flowRunnerSource.Contains('disabledFrame.config.valueFontSize == 20') -and
    $flowRunnerSource.Contains('disabledFrame.config.valueOffsetX == 11') -and
    $flowRunnerSource.Contains('disabledFrame.config.valueOffsetY == -7')
) "Flow covers immediate noncombat resource config and one combat-deferred rebuild"
$buttonThemeFiles = @(
    "UI\Options.lua",
    "UI\AboutPanel.lua",
    "UI\TooltipMonitorMenu.lua",
...
Assert-Contract (
    $harnessLua.IndexOf('loadModule("EventAlertMod/UI/AlertBorderStyles.lua")') -lt $harnessLua.IndexOf('loadModule("EventAlertMod/UI/IconPool.lua")') -and
    $harnessLua.IndexOf('loadModule("EventAlertMod/UI/AboutPanel.lua")') -lt $harnessLua.IndexOf('loadModule("EventAlertMod/UI/Options.lua")')
) "Flow harness UI module load order"
Assert-Contract (
    $harnessLua.IndexOf('loadModule("EventAlertMod/UI/Theme.lua")') -lt $harnessLua.IndexOf('loadModule("EventAlertMod/UI/TooltipMonitorMenu.lua")') -and
    $harnessLua.IndexOf('loadModule("EventAlertMod/UI/Theme.lua")') -lt $harnessLua.IndexOf('loadModule("EventAlertMod/UI/Options.lua")')
) "Flow harness theme module load order"
Assert-Contract (
    $svgProbeIndex -ge 0 -and
    $toc.Contains("EAM_SVG_CAPABILITY_REPORT_JSON") -and
    $harnessLua.Contains('loadModule("EventAlertMod/Debug/SVGCapabilityProbe.lua")')
) "TOC and Flow harness load SVG capability probe"

$packageScriptPath = Join-Path $projectRoot "Deploy\Build-Package.ps1"
$packageScriptText = [System.IO.File]::ReadAllText($packageScriptPath)
$packageUsesExactAddonRoot = $packageScriptText.Contains('$addonRoot = Join-Path $projectRoot "EventAlertMod"') -and
    $packageScriptText.Contains('Get-ChildItem -LiteralPath $addonRoot -Recurse -Force -File') -and
    -not $packageScriptText.Contains('$sourceDirs')
Assert-Contract $packageUsesExactAddonRoot "Package uses complete EventAlertMod source tree"
Assert-Contract (
$packageScriptText.Contains('Managers\AlertManager.lua') -and
$packageScriptText.Contains('Managers\AuraRuleCompiler.lua')
) "Package requires Managers modules"
Assert-Contract (
$packageScriptText.Contains('README.md') -and
$packageScriptText.Contains('changelog.txt')
) "Package requires synchronized README and changelog"
Assert-Contract (
$packageScriptText.Contains('EventAlertMod/Media/SVG') -or
$packageScriptText.Contains('Media\SVG\eam-svg-probe.svg') -or
(Test-Path -LiteralPath $paths.SVGAsset)
) "Package source includes SVG probe asset"

$sourcePackageScriptPath = Join-Path $projectRoot "Deploy\Build-SourcePackage.ps1"
$sourcePackageScriptText = [System.IO.File]::ReadAllText($sourcePackageScriptPath)
Assert-Contract (
    ...
    -not $sourcePackageScriptText.Contains('".codex"') -and
    $sourcePackageScriptText.Contains('EAMSourcePackage_') -and
    $sourcePackageScriptText.Contains('拒絕清理非預期暫存路徑')
) "Source package keeps project settings and excludes only local derivatives"

$deployScriptPath = Join-Path $projectRoot "Deploy\Deploy-EventAlertMod.ps1"
$deployScriptText = [System.IO.File]::ReadAllText($deployScriptPath)
Assert-Contract (
    $deployScriptText.Contains('function Get-RegistryWowCandidates') -and
    $deployScriptText.Contains('[Microsoft.Win32.RegistryView]::Registry64') -and
    $deployScriptText.Contains('[Microsoft.Win32.RegistryView]::Registry32') -and
    $deployScriptText.Contains('DeploymentTargets.json 後備') -and
    $deployScriptText.Contains('Enter 接受；輸入 C 自訂其他主目錄')
) "Deployment discovers Registry root with explicit user override"
Assert-Contract (
    $deployScriptText.Contains('是否同時包含 Retail') -and
    $deployScriptText.Contains('if ($Action -ne "Menu")') -and
    $deployScriptText.Contains('本次候選通道（全部先執行前檢）') -and
    $deployScriptText.Contains('ProductVersion={2}')
) "Deployment confirms PTR or XPTR Retail inclusion and all selected versions"
Assert-Contract (
...
foreach ($deployScriptName in "Build-Package.ps1", "Build-SourcePackage.ps1", "Deploy-EventAlertMod.ps1") {
    $tokens = $null
    $errors = $null
    $scriptPath = Join-Path $projectRoot ("Deploy\" + $deployScriptName)
    [void][System.Management.Automation.Language.Parser]::ParseFile($scriptPath, [ref]$tokens, [ref]$errors)
    Assert-Contract ($errors.Count -eq 0) ("PowerShell AST: Deploy/" + $deployScriptName) (($errors | ForEach-Object { $_.Message }) -join "; ")
}
