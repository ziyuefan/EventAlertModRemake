...
Write-CandidateWarning "CANDIDATE_ONLY: Data/wow_spells_and_auras.json 僅是 Wowhead 網頁候選資料；不是 EAM shipped defaults，也不是 PTR／Retail 實機證據。"

Assert-Contract (Test-Path -LiteralPath $canonicalPath -PathType Leaf) "Canonical candidate JSON exists at Data/wow_spells_and_auras.json"
...
if (Test-Path -LiteralPath $fetchScriptPath -PathType Leaf) {
    ...
    $pathlibOutputPattern = '(?im)^\s*(?:out_file|output_file|output_path)\s*=\s*(?:str\s*\(\s*)?(?:project_root|PROJECT_ROOT)\s*/\s*["'']Data["'']\s*/\s*["'']wow_spells_and_auras\.json["'']'
    $directOsPathOutputPattern = '(?is)os\.path\.join\s*\(\s*(?:project_root|PROJECT_ROOT)\s*,\s*["'']Data["'']\s*,\s*["'']wow_spells_and_auras\.json["'']\s*\)'
    $osDataDirectoryPattern = '(?im)^\s*(?:data_dir|DATA_DIR)\s*=\s*os\.path\.join\s*\(\s*(?:project_root|PROJECT_ROOT)\s*,\s*["'']Data["'']\s*\)'
    ...
    $hasProjectRootAnchor = [regex]::IsMatch($fetchScriptText, $pathlibRootPattern) -or
    ...
    Assert-Contract $hasProjectRootAnchor "Fetch script derives project root from its own location"
    Assert-Contract $hasCanonicalOutput "Fetch script outputs to project_root/Data/wow_spells_and_auras.json"
...
if ($candidateParsed) {
    ...
    if ($hasSpells) {
        ...
        Assert-Contract ($duplicateSpellIDs.Count -eq 0) "spell_id values are unique" (($duplicateSpellIDs.Name | Select-Object -First 20) -join ",")
    ...
    if ($hasClasses) {
        ...
        $classSetMatches = $playerClassIDs.Count -eq $expectedPlayerClassIDs.Count -and
            (Compare-Object -ReferenceObject $expectedPlayerClassIDs -DifferenceObject $playerClassIDs).Count -eq 0
...
$tocFiles = @(Get-ChildItem -LiteralPath $root -File -Filter "*.toc")
Assert-Contract ($tocFiles.Count -gt 0) "At least one root TOC is available for candidate isolation checks"
...
foreach ($tocFile in $tocFiles) {
    ...
    foreach ($line in [System.IO.File]::ReadAllLines($tocFile.FullName, [System.Text.UTF8Encoding]::new($false))) {
        ...
        $luaPath = Join-Path $root $relativeLuaPath
...
Assert-Contract ($tocReferences.Count -eq 0) "Candidate JSON is not referenced by root TOC files" ($tocReferences -join ",")
