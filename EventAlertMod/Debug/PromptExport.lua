...
function PromptExport.buildDetailed()
    ...
    local powerResourceList = {}
    local powerService = EAM.Services and EAM.Services.PlayerResourceService
    local powerStatus = powerService and powerService.getStatus and powerService.getStatus() or nil
    local foregroundPowerType = powerStatus and (powerStatus.foregroundPowerType or powerStatus.activePowerType) or nil
    local foregroundToken = powerStatus and (powerStatus.foregroundToken or powerStatus.activePowerToken) or nil
    local activePowerType = foregroundPowerType
    local activePowerToken = foregroundToken
    local powerResources = powerStatus and powerStatus.resources or nil
    if Util.isReadableTable
        and Util.isReadableTable(powerStatus)
        and Util.isReadableTable(powerResources)
    then
        for index = 1, #powerResources do
            local resource = powerResources[index]
            if Util.isReadableTable(resource)
                and Util.isSafeString(resource.key)
                and Util.isSafeNumber(resource.powerType)
                and Util.isSafeString(resource.token)
                and Util.isSafeString(resource.capability)
                and Util.isSafeBoolean(resource.foreground)
            then
                powerActiveList[#powerActiveList + 1] = tostring(resource.powerType)
                powerResourceList[#powerResourceList + 1] = '{'
                    .. '"key":' .. string.format("%q", resource.key) .. ','
                    .. '"powerType":' .. tostring(resource.powerType) .. ','
                    .. '"token":' .. string.format("%q", resource.token) .. ','
                    .. '"capability":' .. string.format("%q", resource.capability) .. ','
                    .. '"foreground":' .. tostring(resource.foreground)
                    .. '}'
            end
        end
    end
    ...
    add('      "foregroundPowerType": ' .. (
        Util.isSafeNumber and Util.isSafeNumber(foregroundPowerType) and tostring(foregroundPowerType) or "null"
    ) .. ',\n')
    add('      "foregroundToken": ' .. string.format("%q", (
        Util.isSafeString and Util.isSafeString(foregroundToken) and foregroundToken or ""
    )) .. ',\n')
    add('      "activePowerType": ' .. (
        Util.isSafeNumber and Util.isSafeNumber(activePowerType) and tostring(activePowerType) or "null"
    ) .. ',\n')
    add('      "activePowerToken": ' .. string.format("%q", (
        Util.isSafeString and Util.isSafeString(activePowerToken) and activePowerToken or ""
    )) .. ',\n')
    add('      "trackedResourceCount": ' .. tostring(
        Util.isSafeNumber and Util.isSafeNumber(powerStatus and powerStatus.trackedResourceCount)
            and powerStatus.trackedResourceCount or 0
    ) .. ',\n')
    add('      "resources": [' .. table.concat(powerResourceList, ",") .. '],\n')
    add('      "pendingRebuild": ' .. tostring(powerStatus and powerStatus.pendingRebuild == true) .. ',\n')
    add('      "rebuildCount": ' .. tostring(
        Util.isSafeNumber and Util.isSafeNumber(powerStatus and powerStatus.rebuildCount)
            and powerStatus.rebuildCount or 0
    ) .. ',\n')
    add('      "dispatchCount": ' .. tostring(
        Util.isSafeNumber and Util.isSafeNumber(powerStatus and powerStatus.dispatchCount)
            and powerStatus.dispatchCount or 0
    ) .. ',\n')
    add('      "nativeSinkWriteCount": ' .. tostring(
        Util.isSafeNumber and Util.isSafeNumber(powerStatus and powerStatus.nativeSinkWriteCount)
            and powerStatus.nativeSinkWriteCount or 0
    ) .. ',\n')
    add('      "numericUpdateCount": ' .. tostring(
        Util.isSafeNumber and Util.isSafeNumber(powerStatus and powerStatus.numericUpdateCount)
            and powerStatus.numericUpdateCount or 0
    ) .. ',\n')
    add('      "secretDisplayUpdateCount": ' .. tostring(
        Util.isSafeNumber and Util.isSafeNumber(powerStatus and powerStatus.secretDisplayUpdateCount)
            and powerStatus.secretDisplayUpdateCount or 0
    ) .. ',\n')
    add('      "lastResultClass": ' .. string.format("%q", (
        Util.isSafeString and Util.isSafeString(powerStatus and powerStatus.lastResultClass)
            and powerStatus.lastResultClass or "unavailable"
    )) .. ',\n')
    add('      "unavailableUpdateCount": ' .. tostring(
        Util.isSafeNumber and Util.isSafeNumber(powerStatus and powerStatus.unavailableUpdateCount)
            and powerStatus.unavailableUpdateCount or 0
    ) .. ',\n')
