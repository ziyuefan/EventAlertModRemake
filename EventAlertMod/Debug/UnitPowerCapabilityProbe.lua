...
local function updateCase(caseID, role, powerType, sink)
    ...
    if not case.powerTypeAvailable or type(api.UnitPowerPercent) ~= "function"
        or not curve then
    ...
    local ok, percent = pcall(api.UnitPowerPercent, "player", powerType, false, curve)
...
function UnitPowerCapabilityProbe.start()
    ...
    local resourceProbe = EAM.Debug and EAM.Debug.PlayerResourceProbe
    if resourceProbe and type(resourceProbe.start) == "function" then
        local resourceStarted, resourceReason = resourceProbe.start()
...
function UnitPowerCapabilityProbe.stop()
    ...
    UnitPowerCapabilityProbe.update()
    local resourceProbe = EAM.Debug and EAM.Debug.PlayerResourceProbe
    if resourceProbe and type(resourceProbe.stop) == "function" and resourceProbe.isActive() then
        resourceProbe.stop()
    end
