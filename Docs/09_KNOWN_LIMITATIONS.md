...
- VectorGraphics 與 Texture 的 SetSVG secret-argument policy 不相同；兩者不得共用未審查的操作假設。所有 frame／region 建立仍限制在非戰鬥初始化。
- 12.0.7 可安全回報 unsupported；不得為了相容而呼叫不存在的方法。
- 3px 實色邊框已具幾何離線斷言，但不同 UI scale、圖示尺寸與 PTR Native AuraButton 的最終目視仍待玩家截圖簽收。
- SVG ZIP 存在只證明素材被封裝；不證明客戶端解析、顯示、clear/reload 或戰鬥邊界已通過。
