...
#### [Retail 12.1 Alpha 6 Resource Follow-up] 2026.08.21
- 玩家職業資源正式路徑改為獨立的 Catalog、Capability、Service 與 Renderer：涵蓋 17 種資源、13 職業與 40 組職業／專精拓撲，可同時追蹤同專精可用的多種資源，不再只選單一 ClassPower。
- 新增每職業／專精的玩家資源設定面板；每項資源可獨立啟用、排序，並調整前景、背景、數值文字、位置、縮放、透明度、寬高、圖示大小與間距。
- 普通數字可顯示安全文字；Secret 百分比只由 `UnitPowerPercent(..., CurveConstants.ZeroToOne)` 單向送入專用 `StatusBar:SetValue`，不保存、不比較、不字串化、不序列化，也不讀回原生 sink。
- 開發報告改為只輸出資源 key、powerType、token、capability、顯示設定與計數，不包含 current／max／percent 原值。
- 本輪離線 gate：AddOn Lua `61/61`、Flow `70/70`、Validation Contracts `426/426`；Wowhead 候選資料 `23 pass / 0 fail / 4 warnings`。這些結果不等同 Retail／PTR／XPTR 的玩家視覺、戰鬥與專精切換簽收。
...
- Alpha 6 ZIP 改由 `Deploy\Build-Package.ps1` 直接封裝完整 `EventAlertMod/`，並於本機驗證後手動上傳 GitHub prerelease；停用中的 BigWigs／CurseForge／WoWInterface workflow 不啟用。
- 打包器預設先執行 Lua、Flow 與 Validation Contracts；目前打包範圍 gate 為 Lua `57/57`、Flow `69/69`、Validation Contracts `414/414`。另有離線測試 Lua 2 檔，使全樹語法檢查為 `59/59`。這些結果不等同 Retail／PTR／XPTR 真人視覺、音效與戰鬥簽收。
...
- `.AI/Data/wow_spells_and_auras.json` 僅作 Wowhead 離線候選索引，不接入 TOC、runtime 或預設 seeder；唯一治理工具為 `.AI/Tools/Test-WowheadCandidateData.ps1`。
...
- `EventAlertMod/`：唯一 WoW 插件來源；插件安裝包直接封裝整個資料夾，避免漏掉 `Managers/` 或未來新增模組。
- `.AI/`：文件、測試、工具、Schema、爬蟲候選資料、歷史參考、Agent Skills 與中斷接續用 `PROJECT_MEMORY.md`。
- `Deploy/`：插件封裝、完整專案 `src` 封裝與本機部署工具。
- `Dist/`：只存本機 ZIP／SHA-256 產物，已由 `.gitignore` 排除；GitHub 不儲存這些重複二進位內容。
- GitHub Release 提供插件安裝包與專案 `src` 包；`src` 包排除 `Dist/`、Git metadata、備份、測試產物、本機附件與快取。
...
插件包直接封裝完整 `EventAlertMod/`；專案 source 包排除 `Dist/` 與本機衍生物。部署工具會先從 Windows Registry 尋找 WoW 主目錄，找不到時使用設定檔候選，並要求使用者確認或輸入自訂路徑。選擇 PTR／XPTR 時可另外選擇是否連同 Retail 部署；任何目標若為 SymbolicLink、Junction 或其他 Reparse Point，整批部署會停止。建議先執行 `-Action Status` 或對指定通道使用 `-DryRun`，確認版本與目標後再實際部署。
...
玩家資源正式顯示由「圖示位置與能量設定」中的玩家資源面板控制；法力、怒氣、能量、連擊點、瘋狂值、符文等會依目前職業／專精與 `UnitHasPowerType` 能力建立獨立顯示。`/eam test` 的「UnitPower 能力」仍是診斷探針，不等同正式資源面板；玩家需自行產生／消耗資源、切換專精及進出戰鬥，再標記視覺結果。正式開發報告與 `EAM_UNIT_POWER_CAPABILITY_REPORT` 均不得包含 current／max／percent 原值。

```powershell
pwsh -NoProfile -File .\.AI\Tools\Import-EAMFlowReport.ps1 -Path '<EventAlertMod.lua 或 report.json>' -ReportType Auto
```

Mock 通過不代表 Retail／PTR 實機通過。完整規格見 [.AI/Docs/26_FLOW_VALIDATION_FRAMEWORK.md](https://github.com/ziyuefan/EventAlertModRemake/blob/main/.AI/Docs/26_FLOW_VALIDATION_FRAMEWORK.md)。
...
- 離線 mock 通過不等於 PTR 實機通過。完整架構、限制與簽收項目見 `.AI/Docs/23_AURA_CONTAINER_IMPLEMENTATION.md`。
